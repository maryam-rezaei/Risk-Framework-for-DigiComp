...
--- Loading data ---
"""
file = pd.read_excel('vendors.xlsx')
data = file.to_numpy()
inputs = data[:,:9]
outputs = data[:,9]
...
for i in range(epochs):
    for j in range(len(x_train)):
        input = x_train[j] # shape input = (n0,)  --> it is a vector and we need to reshape it to matrix
        input = np.reshape(input , newshape=(1,n0)) # shape input = (1,n0)
        target = Y_train[j]  # target: the desired output we expect

        y1 , y2 , y3 = feedforward(input)
        error = target - y3

        # w1 = w1 - lr * (-2/N)*(error) * d_f3 * w3 * d_f2 * w2 * d_f1 * ...
        # ... * input   ==temp6 in the main code below
        # (-2/N) * error : N-->1 per each vendor we will correct the weights , N-->10 batch learning
        # w1.shape = (n1 , n0)
        # d_f3.shape = (1,n3) = (1,1)
        ...
        # matrix1 * diagonal(d_f1) = matrix2 --> shape = (1, n1) --> matrix2.T --> shape=(n1,1)
        # input.shape = (1 , n0)
    ...
    Netoutput_Train = []
    target_Train = []
    rnd_Netoutput_Train = []   # rnd :rounding
    for idx in range(len(x_train)):
        input = x_train[idx]
        target = Y_train[idx]
        target_Train.append(target)

        _ , _ , pred = feedforward(input)   # pred: predict
 ...
 # calculating MSE and accuracy for Validation
    Netoutput_val = []
    target_val = []
    rnd_Netoutput_val = []   # rnd :rounding
    ...
    print('epoch' ,i , ' : MSE_Valid = ' , mse_val, '\tAcc_Valid = ', acc_val)
    print('------------------------------------------------------')

import matplotlib.pyplot as plt
plt.figure(figsize= (12 , 12))
plt.subplot(2,1,1)
plt.title('MSE(mean_squared_error)')
...
plt.plot(list_AccValid , label='Acc-Valid')
plt.legend()
plt.xlabel('epochs')
plt.ylabel('accuracy')
